...
encore: yarn-install
	docker run --rm -u $(shell id -u):$(shell id -g) -v $(PROJ_DIR):/app --workdir /app node:10-alpine ./node_modules/.bin/encore production
encore-dev: yarn-install
	docker run --rm -u $(shell id -u):$(shell id -g) -v $(PROJ_DIR):/app --workdir /app node:10-alpine ./node_modules/.bin/encore dev
encore-build: encore
.PHONY: encore encore-dev encore-build

node_modules: yarn.lock package.json
	docker run --rm -u $(shell id -u):$(shell id -g) -v $(PROJ_DIR):/app --workdir /app node:10-alpine yarn install --non-interactive
	touch node_modules
